...
export class HebrewDateFormatter {
	...
	private transliteratedParshaMap: Record<Parsha, string> = {
		...
		[Parsha.KORACH_CHUKAS]: 'Korach Chukas',
	...
	// Add to HebrewDateFormatter class
	 ...
	 */
	public formatDafYomiRange(range: DafYomiRange): string {
		const { daf1, daf2, sameMasechta, isConsecutive } = range;

		if (!sameMasechta) {
			if (this.isHebrewFormat()) {
				return `${daf1.getMasechta()} ${this.formatHebrewNumber(daf1.getDaf())} - ${daf2.getMasechta()} ${this.formatHebrewNumber(daf2.getDaf())}`;
			}
			return `${daf1.getMasechtaTransliterated()} ${daf1.getDaf()} - ${daf2.getMasechtaTransliterated()} ${daf2.getDaf()}`;
		}

		// Same masechta
		if (this.isHebrewFormat()) {
			return `${daf1.getMasechta()} ${this.formatHebrewNumber(daf1.getDaf())} - ${this.formatHebrewNumber(daf2.getDaf())}`;
		}

		if (isConsecutive) {
			return `${daf1.getMasechtaTransliterated()} ${Math.min(daf1.getDaf(), daf2.getDaf())}-${Math.max(daf1.getDaf(), daf2.getDaf())}`;
		}
		return `${daf1.getMasechtaTransliterated()} ${daf1.getDaf()}-${daf2.getDaf()}`;
	}
	 ...
	 * @return A formatted string
	 */
	public formatDafYomiRangeMultiple(range: DafYomiRangeMultiple): string {
		const { dafim, allSameMasechta, isConsecutiveRange, minDaf, maxDaf } = range;
		...
		if (dafim.length === 1) {
			if (this.isHebrewFormat()) {
				return `${dafim[0].getMasechta()} ${this.formatHebrewNumber(dafim[0].getDaf())}`;
			}
			return `${dafim[0].getMasechtaTransliterated()} ${dafim[0].getDaf()}`;
		}

		if (allSameMasechta && isConsecutiveRange) {
			const masechta = dafim[0].getMasechta();
			const masechtaTrans = dafim[0].getMasechtaTransliterated();

			if (this.isHebrewFormat()) {
				const formattedDafim = dafim.map(d => this.formatHebrewNumber(d.getDaf())).join(' ');
				return `${masechta} ${formattedDafim}`;
			}
			return `${masechtaTrans} ${minDaf}-${maxDaf}`;
		}

		// Not all in same masechta or not consecutive
		if (this.isHebrewFormat()) {
			return dafim.map(d => `${d.getMasechta()} ${this.formatHebrewNumber(d.getDaf())}`).join(' - ');
		}
		return dafim.map(d => `${d.getMasechtaTransliterated()} ${d.getDaf()}`).join(' - ');
	}

	public formatDafYerushalmiYomiRange(firstDaf: Daf, lastDaf: Daf) {
		const sameMasechta = firstDaf.getMasechtaNumber() === lastDaf.getMasechtaNumber();

		if (this.hebrewFormat) {
			if (sameMasechta) {
				return `${firstDaf.getMasechta()} ${this.formatHebrewNumber(firstDaf.getDaf())}-${this.formatHebrewNumber(lastDaf.getDaf())}`;
			} else {
				return `${firstDaf.getMasechta()} ${this.formatHebrewNumber(firstDaf.getDaf())} - ${lastDaf.getMasechta()} ${this.formatHebrewNumber(lastDaf.getDaf())}`;
			}
		} else {
			if (sameMasechta) {
				return `${firstDaf.getMasechtaTransliterated()} ${firstDaf.getDaf()}-${lastDaf.getDaf()}`;
			} else {
				return `${firstDaf.getMasechtaTransliterated()} ${firstDaf.getDaf()} - ${lastDaf.getMasechtaTransliterated()} ${lastDaf.getDaf()}`;
			}
		}
	}
}
