...
import { default as JewishDate, rangeDates } from '../JewishDate.ts';
...
export class YerushalmiYomiCalculator {
	...
	// Add to YerushalmiYomiCalculator class
	 ...
	 */
	public static formatDafRange(startDate: JewishDate, endDate: JewishDate, hebrewFormat: boolean = true): { firstDaf: DafYomiYerushalmi; lastDaf: DafYomiYerushalmi } {
		let firstDaf: DafYomiYerushalmi;
		let lastDaf: DafYomiYerushalmi;

		try {
			firstDaf = YerushalmiYomiCalculator.getDafYomiYerushalmi(startDate)!;
			if (!firstDaf) {
				const firstDay = startDate.clone();
				firstDay.forward(Calendar.DATE, 1);
				firstDaf = YerushalmiYomiCalculator.getDafYomiYerushalmi(firstDay)!;
			}
		} catch {
			if (Temporal.PlainDate.compare(endDate.getDate(), YerushalmiYomiCalculator.DAF_YOMI_START_DAY) == -1)
				throw new Error("No date is valid");

			return this.formatDafRange(new JewishDate(YerushalmiYomiCalculator.DAF_YOMI_START_DAY), endDate, hebrewFormat);
		}

		lastDaf = YerushalmiYomiCalculator.getDafYomiYerushalmi(endDate)!;
		if (!lastDaf) {
			const lastDay = startDate.clone();
			lastDay.back();
			lastDaf = YerushalmiYomiCalculator.getDafYomiYerushalmi(lastDay)!;
		}

		/* if (!firstDaf || !lastDaf) {
			return hebrewFormat ? 'אין דף' : 'No Daf';
		} */

		return { firstDaf, lastDaf };
	}
}
