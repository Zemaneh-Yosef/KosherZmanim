 ...
 */
export interface DafYomiRangeMultiple {
 ...
 */
export interface DafNumbersRange {
 ...
 * @version 0.0.1
 */
export default class DafYomiCalculator {
	/**
	...
	private static readonly dafYomiJulianStartDay: number = DafYomiCalculator.getJulianDay(DafYomiCalculator.dafYomiStartDate);
	 ...
	 */
	private static readonly shekalimJulianChangeDay: number = DafYomiCalculator.getJulianDay(DafYomiCalculator.shekalimChangeDate);
	...
	public static getDafYomiBavli(calendar: JewishDate): DafBavliYomi {
		...
		let dafNo: number = 0;
		if (Temporal.PlainDate.compare(date, DafYomiCalculator.dafYomiStartDate) == -1) {
			// TODO: should we return a null or throw an IllegalArgumentException?
			throw new IllegalArgumentException(`${calendar} is prior to organized Daf Yomi Bavli cycles that started on ${DafYomiCalculator.dafYomiStartDate}`);
		}
		if (Temporal.PlainDate.compare(date, DafYomiCalculator.shekalimChangeDate) == 1) {
			cycleNo = 8 + ((julianDay - DafYomiCalculator.shekalimJulianChangeDay) / 2711);
			dafNo = ((julianDay - DafYomiCalculator.shekalimJulianChangeDay) % 2711);
		} else {
			cycleNo = 1 + ((julianDay - DafYomiCalculator.dafYomiJulianStartDay) / 2702);
			dafNo = ((julianDay - DafYomiCalculator.dafYomiJulianStartDay) % 2702);
		}
	 ...
	 */
	public static getDafYomiRange(date1: JewishDate, date2: JewishDate): DafYomiRange {
		const daf1 = DafYomiCalculator.getDafYomiBavli(date1);
		const daf2 = DafYomiCalculator.getDafYomiBavli(date2);

		return {
			daf1,
			daf2,
			sameMasechta: daf1.getMasechta() === daf2.getMasechta(),
			isConsecutive: Math.abs(daf2.getDaf() - daf1.getDaf()) === 1,
			dafDifference: Math.abs(daf2.getDaf() - daf1.getDaf()),
			totalPages: Math.abs(daf2.getDaf() - daf1.getDaf()) + 1
		};
	}
	 ...
	 */
	public static getDafYomiRangeMultiple(dates: JewishDate[]): DafYomiRangeMultiple {
		if (dates.length === 0) {
			return { dafim: [], allSameMasechta: true };
		}

		const dafim = dates.map(date => DafYomiCalculator.getDafYomiBavli(date));

		const firstMasechta = dafim[0].getMasechta();
		const allSameMasechta = dafim.every(d => d.getMasechta() === firstMasechta);

		const dafNumbers = dafim.map(d => d.getDaf());
		const minDaf = Math.min(...dafNumbers);
		const maxDaf = Math.max(...dafNumbers);
		...
		return {
			...
			minDaf,
			maxDaf,
			isConsecutiveRange: maxDaf - minDaf === dafim.length - 1,
			totalPages: maxDaf - minDaf + 1
		};
	}
	 ...
	 * @throws IllegalArgumentException if either date is prior to the Daf Yomi start date
	 */
	public static getDafNumbersRange(date1: JewishDate, date2: JewishDate): DafNumbersRange {
		const daf1 = DafYomiCalculator.getDafYomiBavli(date1);
		const daf2 = DafYomiCalculator.getDafYomiBavli(date2);

		const dafNum1 = daf1.getDaf();
		const dafNum2 = daf2.getDaf();

		return {
			dafNum1,
			dafNum2,
			sameMasechta: daf1.getMasechta() === daf2.getMasechta(),
			masechtaName: daf1.getMasechta(),
			masechtaTransliterated: daf1.getMasechtaTransliterated(),
			dafDifference: Math.abs(dafNum2 - dafNum1),
			totalPages: Math.abs(dafNum2 - dafNum1) + 1,
			isConsecutive: Math.abs(dafNum2 - dafNum1) === 1,
			minDaf: Math.min(dafNum1, dafNum2),
			maxDaf: Math.max(dafNum1, dafNum2)
		};
	}
}
