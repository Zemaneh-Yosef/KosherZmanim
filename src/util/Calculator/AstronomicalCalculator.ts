...
export abstract class AstronomicalCalculator {
	// ---- constants ----
	private static readonly GEOMETRIC_ZENITH = 90;

	// ---- configurable properties ----
	private refraction: number = 34 / 60;          // 34′ in degrees
	private solarRadius: number = 16 / 60;         // fixed 16′ (used only when useApparentSolarRadius=false)
	private earthRadius: number = 6371.0088;       // IUGG mean radius (km) – corrected!

	// ---- apparent solar radius toggle ----
	private useApparentSolarRadius: boolean = true;

	// ---- static trig helpers (in degrees) ----
	...
	// ---- Getters / Setters ----
	public getRefraction(): number { return this.refraction; }
	public setRefraction(r: number): void { this.refraction = r; }

	public getSolarRadius(): number { return this.solarRadius; }
	public setSolarRadius(r: number): void {
		if (r < 0 || isNaN(r)) throw new Error('Solar radius must be non‑negative');
		this.solarRadius = r;
		this.useApparentSolarRadius = false;   // as in Java
	}

	public getEarthRadius(): number { return this.earthRadius; }
	public setEarthRadius(r: number): void { this.earthRadius = r; }

	public isUseApparentSolarRadius(): boolean { return this.useApparentSolarRadius; }
	public setUseApparentSolarRadius(flag: boolean): void { this.useApparentSolarRadius = flag; }

	// ---- Apparent solar radius table (365 entries, reference year 2050) ----
	// Copied verbatim from Java AstronomicalCalculator
	private static readonly SOLAR_RADIUS_BY_DAY_OF_YEAR: number[] = [
	 ...
	 */
	public getApparentSolarRadius(date: Temporal.PlainDate | null): number {
		if (!date || !this.useApparentSolarRadius) return 16 / 60;
		// Map Feb 29 to Feb 28 by forcing year 2050 (common year)
		const doy = date.with({ year: 2050 }).dayOfYear;
		return AstronomicalCalculator.SOLAR_RADIUS_BY_DAY_OF_YEAR[doy - 1];
	}

	// ---- Elevation adjustment ----
	public getElevationAdjustment(elevationMeters: number): number {
		const earthRadiusKm = this.earthRadius;
		return Math.acos(earthRadiusKm / (earthRadiusKm + elevationMeters / 1000)) * 180 / Math.PI;
	}
	 ...
	 */
	public adjustZenith(zenith: number, elevation: number, date?: Temporal.PlainDate | null): number {
		if (zenith !== AstronomicalCalculator.GEOMETRIC_ZENITH) return zenith;
		const radius = this.getApparentSolarRadius(date ?? null);
		return zenith + radius + this.getRefraction() + this.getElevationAdjustment(elevation);
	}

	// ---- abstract methods (to be implemented by subclasses) ----
	public abstract getCalculatorName(): string;
	public abstract getUTCSunrise(date: Temporal.PlainDate, geoLocation: GeoLocation, zenith: number, adjustForElevation: boolean): number;
