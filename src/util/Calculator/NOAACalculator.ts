...
export class NOAACalculator extends AstronomicalCalculator {
	...
	private static getSolarNoonMidnightUTC(julianDay: number, longitude: number, solarEvent: SolarEvent): number {
		...
		const solNoonUTC: number = (longitude * 4) - eqTime; // min
		...
		return (solarEvent == SolarEvent.NOON ? 720 : 1440) + (longitude * 4) - eqTime; // min
	}
	...
	private static getSunRiseSetUTC(calendar: Temporal.PlainDate, latitude: number, longitude: number, zenith: number, solarEvent: SolarEvent): number {
		...
		const noonmin: number = NOAACalculator.getSolarNoonMidnightUTC(julianDay, longitude, SolarEvent.NOON);
