 ...
 */
export default class SPACalculator extends AstronomicalCalculator {
  ...
  private temperature: number = 10.0;
   ...
   * @see #isApplyDeltaT()
   */
  public setApplyDeltaT(applyDeltaT: boolean): void {
   ...
   */
  public setDeltaTOverride(deltaTSeconds: number | null): void {
   ...
   */
  public getDeltaTOverride(): number | null {
   ...
   */
  public setPressure(pressureMillibars: number): void {
   ...
   */
  public setTemperature(temperatureCelsius: number): void {
   ...
   */
  private getUTCSunRiseSet(localDate: Temporal.PlainDate, geoLocation: GeoLocation, zenith: number,
      adjustForElevation: boolean, solarEvent: string): number {
    const elevation = adjustForElevation ? geoLocation.getElevation() : 0;
    const adjustedZenith = this.adjustZenith(zenith, elevation, localDate);
    let riseSet = this.solveRiseSet(localDate, geoLocation, adjustedZenith, solarEvent);
    if (Number.isNaN(riseSet)) {
      return Number.NaN;
    }
    riseSet = riseSet / 60;
   ...
   */
  private solveRiseSet(localDate: Temporal.PlainDate, geoLocation: GeoLocation, adjustedZenith: number, solarEvent: string): number {
    const jdDay = SPACalculator.getJulianDay(localDate);
    const lonWest = -geoLocation.getLongitude();
    const lat = geoLocation.getLatitude();
    const elevation = geoLocation.getElevation();

    // Closed-form first guess using the geocentric declination at local apparent noon.
    const noonMin = this.solveNoonMidnight(jdDay, lonWest, SolarEvent.NOON);
    const noonCoords = this.solarCoords(jdDay + noonMin / 1440.0);
    const declNoon = noonCoords[1];
    const eot = this.equationOfTime(jdDay + noonMin / 1440.0);
    const cosH0 = (AstronomicalCalculator.cosDegrees(adjustedZenith) - AstronomicalCalculator.sinDegrees(lat) * AstronomicalCalculator.sinDegrees(declNoon))
        / (AstronomicalCalculator.cosDegrees(lat) * AstronomicalCalculator.cosDegrees(declNoon));
    if (cosH0 < -1.0 || cosH0 > 1.0) {
      return Number.NaN; // sun never reaches this zenith (polar day / night)
    }
    const h0 = AstronomicalCalculator.acosDegrees(cosH0); // degrees, positive
    const signed = (solarEvent === SolarEvent.SUNRISE) ? h0 : -h0;
    const guess = 720 + 4 * (lonWest - signed) - eot; // minutes

    // Secant refinement on f(t) = topocentricTrueZenith(t) - adjustedZenith.
    let t0 = guess;
    let t1 = guess + 0.5;
    let f0 = this.topocentricTrueZenith(jdDay + t0 / 1440.0, lat, geoLocation.getLongitude(), elevation) - adjustedZenith;
    let f1 = this.topocentricTrueZenith(jdDay + t1 / 1440.0, lat, geoLocation.getLongitude(), elevation) - adjustedZenith;
    for (let i = 0; i < 12 && Math.abs(f1) > 1e-9; i++) {
      const denom = (f1 - f0);
      if (denom === 0) {
        break;
      }
      ...
      t0 = t1; f0 = f1;
      t1 = t2;
      f1 = this.topocentricTrueZenith(jdDay + t1 / 1440.0, lat, geoLocation.getLongitude(), elevation) - adjustedZenith;
    }
    return t1;
  }

  public getUTCNoon(localDate: Temporal.PlainDate, geoLocation: GeoLocation): number {
    let noon = this.solveNoonMidnight(SPACalculator.getJulianDay(localDate), -geoLocation.getLongitude(), SolarEvent.NOON) / 60;
    return (noon % 24 + 24) % 24;
  }

  public getUTCMidnight(localDate: Temporal.PlainDate, geoLocation: GeoLocation): number {
    let midnight = this.solveNoonMidnight(SPACalculator.getJulianDay(localDate), -geoLocation.getLongitude(), SolarEvent.MIDNIGHT) / 60;
   ...
   */
  private solveNoonMidnight(julianDay: number, lonWest: number, solarEvent: string): number {
    ...
    const lonEast = -lonWest;
    const targetHourAngle = (solarEvent === SolarEvent.NOON) ? 0.0 : 180.0;
    let dayFraction = ((solarEvent === SolarEvent.NOON) ? 0.5 : 1.0) - lonEast / 360.0;
    for (let i = 0; i < 3; i++) {
      const sc = this.solarCoords(julianDay + dayFraction);
      const alpha = sc[0]; // apparent right ascension (degrees)
      const nu = sc[3];    // apparent sidereal time at Greenwich (degrees)
      const hourAngle = ((nu + lonEast - alpha - targetHourAngle) % 360 + 540) % 360 - 180; // [-180, 180)
  ...
  public getTimeAtAzimuth(localDate: Temporal.PlainDate, geoLocation: GeoLocation, targetAzimuth: number): number {
    ...
    const julianDay = SPACalculator.getJulianDay(localDate);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const decl = this.solarCoords(jd)[1];
      const ratio = AstronomicalCalculator.tanDegrees(decl) / AstronomicalCalculator.tanDegrees(geoLocation.getLatitude());
      if (Number.isNaN(ratio) || ratio > 1.0 || ratio < -1.0) {
        return Number.NaN;
      }
    ...
    const timeUTC = dateTime * 24.0;
    return (timeUTC % 24 + 24) % 24;
  }

  public getSolarElevation(instant: Temporal.Instant, geoLocation: GeoLocation): number {
    const jd = SPACalculator.julianDayFromInstant(instant);
    const topo = this.topocentric(jd, geoLocation.getLatitude(), geoLocation.getLongitude(), geoLocation.getElevation());
    return topo[1]; // observed (refraction-corrected) topocentric elevation
  }

  public getSolarAzimuth(instant: Temporal.Instant, geoLocation: GeoLocation): number {
    const jd = SPACalculator.julianDayFromInstant(instant);
    const topo = this.topocentric(jd, geoLocation.getLatitude(), geoLocation.getLongitude(), geoLocation.getElevation());
   ...
   */
  private solarCoords(julianDayUT: number): number[] {
    const deltaTSeconds = this.applyDeltaT ? (this.deltaTOverride !== null ? this.deltaTOverride : SPACalculator.estimateDeltaT(julianDayUT)) : 0.0;
    const jde = julianDayUT + deltaTSeconds / 86400.0;
    const jce = (jde - SPACalculator.JULIAN_DAY_JAN_1_2000) / SPACalculator.JULIAN_DAYS_PER_CENTURY;
    const jc = (julianDayUT - SPACalculator.JULIAN_DAY_JAN_1_2000) / SPACalculator.JULIAN_DAYS_PER_CENTURY;
    const jme = jce / 10.0;

    const earthL = MathUtils.radiansToDegrees(SPACalculator.sumSeries(SPACalculator.EARTH_L, jme));
    const earthB = MathUtils.radiansToDegrees(SPACalculator.sumSeries(SPACalculator.EARTH_B, jme));
    const radius = SPACalculator.sumSeries(SPACalculator.EARTH_R, jme); // AU

    const theta = (earthL + 180.0) % 360.0;   // geocentric longitude
    const beta = -earthB;                      // geocentric latitude

    const nut = SPACalculator.nutation(jce);               // [Δψ, Δε] in degrees
    const epsilon = SPACalculator.meanObliquity(jme) + nut[1];
    const aberration = -20.4898 / (3600.0 * radius); // degrees
    const lambda = theta + nut[0] + aberration;      // apparent longitude

    // apparent sidereal time at Greenwich
    let nu0 = 280.46061837 + 360.98564736629 * (julianDayUT - SPACalculator.JULIAN_DAY_JAN_1_2000)
        + 0.000387933 * jc * jc - jc * jc * jc / 38710000.0;
    nu0 = (nu0 % 360 + 360) % 360;
    const nu = nu0 + nut[0] * AstronomicalCalculator.cosDegrees(epsilon);

    const alpha = MathUtils.radiansToDegrees(Math.atan2(
        AstronomicalCalculator.sinDegrees(lambda) * AstronomicalCalculator.cosDegrees(epsilon) - AstronomicalCalculator.tanDegrees(beta) * AstronomicalCalculator.sinDegrees(epsilon),
        AstronomicalCalculator.cosDegrees(lambda)));
    const alphaAdjusted = (alpha % 360 + 360) % 360;
    const delta = AstronomicalCalculator.asinDegrees(AstronomicalCalculator.sinDegrees(beta) * AstronomicalCalculator.cosDegrees(epsilon)
        + AstronomicalCalculator.cosDegrees(beta) * AstronomicalCalculator.sinDegrees(epsilon) * AstronomicalCalculator.sinDegrees(lambda));

    return [alphaAdjusted, delta, epsilon, nu, radius, lambda];
  }
   ...
   */
  private topocentric(julianDayUT: number, latitude: number, longitude: number, elevationMeters: number): number[] {
    const sc = this.solarCoords(julianDayUT);
    const alpha = sc[0], delta = sc[1], nu = sc[3], radius = sc[4];

    let h = (nu + longitude - alpha) % 360.0; // observer local hour angle (deg), positive west
    h = (h + 360) % 360;

    // equatorial horizontal parallax of the sun
    const xi = 8.794 / (3600.0 * radius); // degrees
    const u = Math.atan(0.99664719 * AstronomicalCalculator.tanDegrees(latitude));
    const x = Math.cos(u) + (elevationMeters / 6378140.0) * AstronomicalCalculator.cosDegrees(latitude);
    const y = 0.99664719 * Math.sin(u) + (elevationMeters / 6378140.0) * AstronomicalCalculator.sinDegrees(latitude);

    const deltaAlpha = MathUtils.radiansToDegrees(Math.atan2(-x * AstronomicalCalculator.sinDegrees(xi) * AstronomicalCalculator.sinDegrees(h),
        AstronomicalCalculator.cosDegrees(delta) - x * AstronomicalCalculator.sinDegrees(xi) * AstronomicalCalculator.cosDegrees(h)));
    const deltaPrime = MathUtils.radiansToDegrees(Math.atan2(
        (AstronomicalCalculator.sinDegrees(delta) - y * AstronomicalCalculator.sinDegrees(xi)) * AstronomicalCalculator.cosDegrees(deltaAlpha),
        AstronomicalCalculator.cosDegrees(delta) - x * AstronomicalCalculator.sinDegrees(xi) * AstronomicalCalculator.cosDegrees(h))); // topocentric declination
    const hPrime = h - deltaAlpha; // topocentric local hour angle

    const e0 = AstronomicalCalculator.asinDegrees(AstronomicalCalculator.sinDegrees(latitude) * AstronomicalCalculator.sinDegrees(deltaPrime)
        + AstronomicalCalculator.cosDegrees(latitude) * AstronomicalCalculator.cosDegrees(deltaPrime) * AstronomicalCalculator.cosDegrees(hPrime)); // true (no refraction)

    const deltaE = this.refractionCorrection(e0); // degrees
    const e = e0 + deltaE;                    // observed elevation

    const gamma = MathUtils.radiansToDegrees(Math.atan2(AstronomicalCalculator.sinDegrees(hPrime),
        AstronomicalCalculator.cosDegrees(hPrime) * AstronomicalCalculator.sinDegrees(latitude) - AstronomicalCalculator.tanDegrees(deltaPrime) * AstronomicalCalculator.cosDegrees(latitude)));
    let azimuth = (gamma + 180.0) % 360.0; // eastward from north
   ...
   */
  private topocentricTrueZenith(julianDayUT: number, latitude: number, longitude: number, elevationMeters: number): number {
   ...
   */
  private refractionCorrection(trueElevation: number): number {
    const atmosRefract = this.getRefraction(); // parent's configurable horizon refraction (degrees), default 34'
    ...
    const sunRadius = this.getSolarRadius();
    ...
    return (this.pressure / 1010.0) * (283.0 / (273.0 + this.temperature))
        * 1.02 / (60.0 * AstronomicalCalculator.tanDegrees(trueElevation + 10.3 / (trueElevation + 5.11)));
  }
   ...
   */
  private equationOfTime(julianDayUT: number): number {
    const sc = this.solarCoords(julianDayUT);
    const alpha = sc[0], epsilon = sc[2];
    const deltaTSeconds = this.applyDeltaT ? (this.deltaTOverride !== null ? this.deltaTOverride : SPACalculator.estimateDeltaT(julianDayUT)) : 0.0;
    const jme = ((julianDayUT + deltaTSeconds / 86400.0) - SPACalculator.JULIAN_DAY_JAN_1_2000) / SPACalculator.JULIAN_DAYS_PER_CENTURY / 10.0;
    let l0 = 280.4664567 + jme * (360007.6982779 + jme * (0.03032028
        + jme * (1.0 / 49931.0 - jme * (1.0 / 15300.0 + jme / 2000000.0))));
    l0 = (l0 % 360 + 360) % 360;
    const deltaPsi = SPACalculator.nutation(((julianDayUT + deltaTSeconds / 86400.0) - SPACalculator.JULIAN_DAY_JAN_1_2000) / SPACalculator.JULIAN_DAYS_PER_CENTURY)[0];
    let e = l0 - 0.0057183 - alpha + deltaPsi * AstronomicalCalculator.cosDegrees(epsilon); // degrees
   ...
   */
  private static julianDayFromInstant(instant: Temporal.Instant): number {
    const utc = instant.toZonedDateTimeISO('UTC');
    const fractionalDay = (utc.hour + (utc.minute
        + (utc.second + utc.millisecond / 1000.0) / 60.0) / 60.0) / 24.0;
    return SPACalculator.getJulianDay(utc.toPlainDate()) + fractionalDay;
  }
   ...
   */
  private static meanObliquity(jme: number): number {
    const u = jme / 10.0;
    const seconds = 84381.448 - 4680.93 * u - 1.55 * u * u + 1999.25 * Math.pow(u, 3) - 51.38 * Math.pow(u, 4)
   ...
   */
  private static nutation(jce: number): number[] {
    const t = jce;
    ...
    const deltaPsiArcsec = -17.20 * AstronomicalCalculator.sinDegrees(omega) - 1.32 * AstronomicalCalculator.sinDegrees(2 * lSun)
        - 0.23 * AstronomicalCalculator.sinDegrees(2 * lMoon) + 0.21 * AstronomicalCalculator.sinDegrees(2 * omega);
    const deltaEpsArcsec = 9.20 * AstronomicalCalculator.cosDegrees(omega) + 0.57 * AstronomicalCalculator.cosDegrees(2 * lSun)
        + 0.10 * AstronomicalCalculator.cosDegrees(2 * lMoon) - 0.09 * AstronomicalCalculator.cosDegrees(2 * omega);
   ...
   */
  private static estimateDeltaT(julianDay: number): number {
    const year = 2000.0 + (julianDay - SPACalculator.JULIAN_DAY_JAN_1_2000) / 365.25;
    let t: number;
   ...
   */
  private static sumSeries(series: number[][][], tau: number): number {
    let total = 0.0;
    let tauPow = 1.0;
  ...
  /*public equals(object: object): boolean {
    ...
    const other = object as SPACalculator;
    return this.applyDeltaT === other.applyDeltaT
        && this.deltaTOverride === other.deltaTOverride
  ...
  public hashCode(): number {
    ...
    hash = hash * 31 + (this.applyDeltaT ? 1 : 0);
    hash = hash * 31 + (this.deltaTOverride !== null ? this.deltaTOverride.valueOf() : 0);
    hash = hash * 31 + this.pressure.valueOf();
    hash = hash * 31 + this.temperature.valueOf();
    return hash;
  }*/
  ...
  private static readonly EARTH_L: number[][][] = [
    [ // L0
  ...
  private static readonly EARTH_B: number[][][] = [
    [ // B0
  ...
  private static readonly EARTH_R: number[][][] = [
    [ // R0
