...
describe('Test YerushalmiYomiCalculator', function () {
  it('Gets Yerushalmi daf for 10 Elul 5783 (Kilayim 44)', () => {
    const jewishCalendar: JewishCalendar = new JewishCalendar(5783, JewishDate.ELUL, 10);
    const daf: Daf = YerushalmiYomiCalculator.getDafYomiYerushalmi(jewishCalendar)!;

    assert.strictEqual(daf.getMasechtaNumber(), 3);
    assert.strictEqual(daf.getDaf(), 43);
  });
